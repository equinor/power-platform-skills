...
> **Equinor fork:** the upstream telemetry stack is **not** shipped here. `shared/telemetry/`, the
> per-plugin `scripts/lib/telemetry/` copies, the telemetry hooks, and the `/telemetry` skills are
> excluded pending a separate governance and privacy review, so no plugin in this fork emits usage
> telemetry. The rest of this section describes the upstream design and is kept as reference for
> that review and for future syncs \u2014 it does not describe files that exist in this repository.
