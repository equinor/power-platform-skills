...
```bash
curl -fsSL https://raw.githubusercontent.com/equinor/power-platform-skills/main/marketplace.json 2>/dev/null
```

Find the plugin entry whose `name` matches the local plugin name, read its `source`, then fetch the matching remote plugin manifest at `<source>/.plugin/plugin.json` and parse its `version` field.
