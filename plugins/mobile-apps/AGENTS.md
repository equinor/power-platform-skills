...
> **Equinor fork:** the telemetry stack described below is **not** shipped here.
> `scripts/lib/telemetry/`, `scripts/lib/mobile-telemetry*.js`, `hooks/run-telemetry.js`,
> `scripts/emit-telemetry-checkpoint.js`, and the `/telemetry` skill are excluded pending a
> separate governance and privacy review. Checkpoint calls in the skill workflows are inert.
