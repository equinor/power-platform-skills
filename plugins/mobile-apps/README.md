...
1. Create a new app from the template and install dependencies:
    ...
    ```sh
    npx degit equinor/power-platform-skills/plugins/mobile-apps/template#main my-mobile-app
    cd my-mobile-app
    npm install
    ```
...
2. Install the mobile-app plugin from the Power Platform Skills marketplace.
    ...
    ```sh
    copilot plugin marketplace add equinor/power-platform-skills
    ...
    ```sh
    claude plugin marketplace add equinor/power-platform-skills
...
library, hooks, checkpoint emitters, and `/telemetry` skill are excluded pending a
separate governance and privacy review, so this plugin sends no usage telemetry and
writes no local telemetry mirror.

## Known blockers

## See also

- [`plugins/mobile-apps/template`](https://github.com/equinor/power-platform-skills/tree/main/plugins/mobile-apps/template) — bundled Expo standalone template and fresh-template working directory source
