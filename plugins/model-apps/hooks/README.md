...
> **Equinor fork:** upstream also wires a `Skill\|skill` PreToolUse hook
> (`run-skill-pretool-telemetry.js`) and a `UserPromptSubmit` hook
> (`run-user-prompt-telemetry.js`) here, emitting `skill_started` usage telemetry. Both are
> intentionally not wired in this fork because the 1DS telemetry stack is excluded pending a
> separate governance and privacy review.
